...
1. **Check state.** Read `template.config.json`. If `"initialized": true`, tell the user the repo
   ...
   proceeding.
2. **Interview.** Walk the 12 questions from the skill, one at a time, showing the current default
   and accepting it on empty input. Keep it brisk.
3. **Confirm.** Echo a compact summary table and get an explicit go-ahead.
4. **Preview.** Run `scripts/Initialize-Template.ps1` with the gathered parameters and `-WhatIf`,
   and show the user what would change.
5. **Apply.** On approval, re-run the same command without `-WhatIf`.
6. **Hand off.** Summarize the manual follow-ups (new `app.json` GUID, target BC version, adjust
